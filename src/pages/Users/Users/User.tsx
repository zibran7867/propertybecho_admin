...
const User = () => {
  ...
  const getAllUser = async (filter?: AreaFilter) => {
    await userService
      .getAllUsers(
      ...
      .then((response) => {
        ...
        console.log("Users responseData", response)
        setData(responseData.data.rows)
        const { data } = response.data;
        console.log("🚀 ~ getAllUsers ~ status:", data)
        if (data) {
  ...
  const handleSetPageDetails = async (filter: AreaFilter, pageDetail: Pagination) => {
    ...
    await getAllUser(filter || initialState);
  };

  const debouncedResults = React.useMemo(() => {
      return debounce(getAllUser, 300);
    }, []);
