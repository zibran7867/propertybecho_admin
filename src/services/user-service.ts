...
const endPointBaseURLUser = `user`;
...
const getAllUsers = async (
    ...
    httpService.get<ApiResponseModel<any>>(`${endPointBaseURLUser}/all/?status=${status ?? ""}&search=${search ?? ''}&limit=${pagination?.limit ?? 10}&page=${pagination?.page ?? 1}&sortOrder=${sort?.direction ? Sorting[sort?.direction] : ""}`);
